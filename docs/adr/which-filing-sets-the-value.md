...
M4-19 landed on 2026-09-10, runs the attempt inside every answering filing and
then stops: it orders no two filings, parses no `filed`, and reads no form, saying
in its own header that the record states the rule twice in ways that do not agree
and that this is the decider's to settle. So this is not a second revision
changing values a first revision stored. It is the first statement of Rule 3 that
...
**`contracts/fetch-normalize/v2.toml`**, the version the accepted record proposed,
published since. Nine fields per fact. `form` and `filed` cross for this ruleset
and have no other consumer; `report_period_end`, the field v2 added, crosses for
the accepted record's cover-page condition, and this rule does not read it. Every
value crosses as the characters the document publishes, unparsed. Normalize reads
those characters into an exact decimal where a `sum` or a `difference` asks it to
...
What is readable is the merged fetch fixtures — three carry a filer's facts, and
two of them hold the cases below — and `registry/`, whose 74 elements are the same
set since #137 landed them on 2026-09-02 — two days before the accepted record was
proposed, and #155 added only `reading` and `answers`.
...
over one quoting it in a comparative column — which `report_period_end` now makes
stateable, and which is argued under Alternatives. `filed` is compared as a date,
which is where the fetch record already puts that parse.
...
- **Latest-that-changed: the value is the latest figure, and it names the earliest
  ...
  composition renders at its own scale. No merged fixture holds two literals
  spelling one number today, so this is a hole rather than a live failure, which
  is exactly when it is cheap to refuse. **It is undefined where a value changes
  and changes back.** `current_liabilities` at 2023-12-31 on CIK 0001859199 is stated 2,062,834 by the
...
- **Prefer the filing whose attempt read the concept exactly over one that stood
  ...
  the filing with more facts, or the one below) each of which reads well and none
  of which is a date.
- **Prefer the filing that reports the period as its own, and only then take the
  greatest `filed`.** At `fetch-normalize` v2 it is stateable — a filing whose
  `report_period_end` is the period's end — and it is what a reader expects of a
  period's value: the figure from the report about that period rather than from a
  column quoting it. Rejected on the two fixtures above that hold a restatement,
  because on both it keeps exactly the figures the filer took back. At the
  half-year 2023-10-01 to 2024-03-31 the filing reporting the period as its own is
  the 10-Q filed 2024-05-08, whose `report_period_end` is 2024-03-31, so
  `earnings_per_share_diluted` would stay at −0.002 against the −0.0015 the filer
  corrected it to a year later. At the transition period 2023-05-01 to 2023-12-31
  it is the 10-KT, so `operating_income` would stay at −6,338,622 and
  `depreciation_and_amortization` at 289,067, which the 10-K and the 10-K/A both
  restated. In both cases the restatement arrives in a
  filing about a later period, so a preference for the period's own report is a
  preference against the restatement; and the preference reads a field that is not
  `filed`, `form` or `accession`.
...
mixes filings by construction — the fixture's quarter names two of them and, for
...
silence zero is not one. No merged fixture exercises it: none of the three that
carry a filer's facts holds a fact under an element the registry maps to any of
the three concepts, and no period in any of them holds a silence value beside a
read one. A clause for it would prefer an attempt by how it was set, which is not
`filed`, `form` or `accession`, so it is a decision of its own and belongs in its
own record — most likely beside the vocabulary's silence readings rather than
here. The queue holds that record as `M4-30`.
...
- **The tie clause is unexercised.** No two filings share a `filed` date in any
  merged fixture — ten filings for CIK 0002003750, fourteen for CIK 0001859199,
  twenty-one for CIK 0001715819. The nearest is CIK 0001859199's 10-Q filed
  2025-08-14 and the 10-Q/A for the same period filed the next day, which `filed`
  orders without the clause. So both halves of it, the `/A` tie-break and the
  surviving tie that resolves to `Unknown` carrying both accessions, are stated
  and unchecked, as is the condition this
