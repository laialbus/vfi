...
| `what-normalize-emits.md`, the tie sentence of its `Unknown` section, and `canonical-concepts` v2's `Unknown` line as read at a tie | `tie-and-undated-cross-into-v2.md` | Tied filings cross as attempts beside the tie; an unreadable `filed` is a stop. |
| `golden-claims-and-baseline-re-recording.md`, the proof-of-catch sentence | `benchmark-proof-scales-to-the-baseline.md` | The proof keeps its target and allocates a tenth of the baseline's bytes per call; the baseline states its `passes`. |
| `tie-and-undated-cross-into-v2.md`, "lost on `filed` alone" | the owner, on the 2026-09-25 decider's flag, in the 2026-09-26 lead entry | A filing that lost on `form` beneath a tie on `filed` is not carried either; it is neither failed nor undecided. |
| `worker-terminal-state.md`, one consequence | `escalated-refs-park-their-task.md` | An `escalated/` ref parks its task; the sweep deletes the ref when it folds the file. |
