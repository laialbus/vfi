#!/usr/bin/env python3
r"""Stop gate: a worker run ends only in a terminal state. DRAFT — NOT INSTALLED.

Destination: `.claude/hooks/worker_terminal_state.py`, a protected path, so
installation is a human act. `docs/adr/worker-terminal-state.md` (Accepted,
2026-08-03) is the sign-off; this file is the implementation of that decision
and nothing more. Verify before installing anything:

    python3 docs/proposals/worker_terminal_state_tests.py

Three pieces land together. The file alone does nothing.

1. **The hook** — this file, at `.claude/hooks/worker_terminal_state.py`.

2. **The wiring** — `.claude/settings.json` gains a `Stop` key beside the
   `PreToolUse` one it already has. `Stop` takes no matcher:
       ...
       "Stop": [
         {
           "hooks": [
             {
               "type": "command",
               "command": "python3 \"$CLAUDE_PROJECT_DIR\"/.claude/hooks/worker_terminal_state.py",
               "timeout": 30
             }
           ]
         }
   ...
   The timeout is deliberate. The default for a command hook is 600 seconds
   and this hook reaches the network twice; a hook Claude Code cancels does
   not block, so the timeout is one more path that fails open.

3. **The sentence** — AGENTS.md's finish step (§ "Every run, start to
   finish", item 6) gains the line the ADR names:

       The run ends when your turn ends. A headless session does not outlive
       its final message, so work deferred to a background task or monitor is
       work that never happens. Reach a terminal state — PR open, or
       escalation pushed — before you finish.
