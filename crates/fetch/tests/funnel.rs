...
//! ask for.
//!
//! The golden fixtures pin the two steps against documents recorded from EDGAR.
//! These are the cases a recording cannot be taken of, and the documents below
//! are written here rather than fetched, which is worth saying plainly because a
...
use vfi_fetch::funnel::{Calendar, Sweep, Unswept, admitted, gate, seed};
use vfi_fetch::ledger::{
    Filer, FilerKey, FilerLedger, InMemory, Pass, Reason, Record, Step, Verdict, When,
};
use vfi_fetch::{Cleared, Declaration, Egress, Pace, Response, Transport};
...
/// A metadata document naming the filings given as `(form, filed)`, in the
/// shape the fixtures record. `overflow` is the pages of older filings EDGAR
/// names, which this step must never follow.
fn submissions_of(cik: u64, filings: &[(&str, &str)], overflow: &[&str]) -> String {
    ...
    format!(
        r#"{{"cik":"{cik:010}","entityType":"operating","filings":{{"recent":{{
            "accessionNumber":[{accessions}],"filingDate":[{filed}],"reportDate":[{periods}],
            "form":[{forms}],"primaryDocument":[{documents}]}},"files":[{files}]}}}}"#,
        accessions = column(
            (0..filings.len())
...
/// One pass over the two steps, and what it left behind.
struct Passed {
    ledger: InMemory,
