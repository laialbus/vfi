...
//! stops.
//!
//! Three steps, and the first two are here. [`seed`] reads the filers there are
//! out of what EDGAR publishes. [`gate`] judges each of them on what EDGAR
//! publishes about it, before anything asks what it has filed. The third —
//! retrieving a history for the survivors — is the step after these.
//!
//! Every filer either step reaches leaves a verdict behind, through
//! [`crate::ledger`] and nowhere else. That is not bookkeeping: most of the
//! corpus stops at one of these two steps, and a filer that stops leaves no
//! filings, no metrics and no row anyone can ask about, so the verdict is the
//! whole of the trace that it was ever considered.
//!
//! What one step hands the next is the keys it judged, never which of them it
//! kept. Whether a filer goes on is [`admitted`]'s answer, read back out of the
//! ledger, so a filer reaching the step after this one without a record is not
//! something a caller has to remember to prevent — there is no path that
//! produces one.
